...
const updateVfr = async () => {
    ...
    await axios.post("/api/update_vfr2/" + firstContenu.id, firstContenu).then((response)=>{
...
const updateVen = async () => {
    ...
    await axios.post("/api/update_ven2/" + firstContenu.id, firstContenu).then((response)=>{
        if(response.data.success){
            console.log('ok')
            toast.fire({
            icon: "success",
